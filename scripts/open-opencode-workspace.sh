...
if [ "$PANE_COUNT" -ge 9 ]; then
    tmux display-message "Opencode workspace already exists in this window!"
    exit 0
fi
...
# Send 'opencode' to the 8 newly created panes (indices 1-8) in the current directory
for PANE_IDX in 1 2 3 4 5 6 7 8; do
    tmux send-keys -t "$SESSION:$CURRENT_WINDOW.$PANE_IDX" "cd '$CURRENT_DIR' && opencode" Enter
done
...
tmux display-message "Opencode workspace created: 9 panes in current directory"
